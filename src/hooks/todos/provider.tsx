...
export function TodoContextProvider({ children }: { children: ReactNode }) {
  ...
  const value = {
    ...
    editTodo: (id: number, content: string) => dispatch({ type: ACTIONS.EDIT, id, content }),
    removeTodo: (id: number) => dispatch({ type: ACTIONS.REMOVE, id }),
    setCompleted: (id: number, completed: boolean) => dispatch({ type: ACTIONS.SET_COMPLETED, id, completed }),
  }
